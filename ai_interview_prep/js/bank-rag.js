...
{ id: 'rg57', topic: 'rag', level: 2,
  q: 'A PDF table holds the answer in row 14, column 3. Chunking flattened the table and retrieval fails. How do you fix it?',
  lay: 'The bug is upstream of retrieval. By the time a table has been read as a stream of words, the link between a number and the column heading above it is gone — the chunk says "4,820" and nothing else. Fix it where the document is parsed: detect the table as a table, keep it whole, repeat the headings on every piece, and store where each row came from.',
  tech: 'Six things, in the order they matter:<ul><li><b>Parse layout, not text.</b> A plain text extractor reads a table in reading order and destroys the grid. Use a table-aware extractor — Camelot or Tabula for ruled tables, Unstructured, Docling, Azure Document Intelligence or Textract for real-world ones, and a table-structure model over OCR for scans.</li><li><b>Serialise, do not flatten.</b> Emit the table as Markdown or HTML so the header row survives as a header row. Models read a Markdown table well; they cannot read a column of orphan numbers.</li><li><b>Never split mid-table.</b> Make the table its own chunk. If it exceeds the chunk budget, split by rows and <b>repeat the header row plus the caption and heading path on every part</b> — that one rule fixes most of these failures on its own.</li><li><b>Dual-index the rows.</b> Alongside the table chunk, index each row as a sentence: "SKU-4471 — region: EMEA, Q3 revenue: 4,820". Lookup questions match a row; the table chunk gives the model the surrounding structure.</li><li><b>Carry coordinates in metadata</b> — table id, page, row range, column names — so a citation can say "row 14, Revenue" instead of "somewhere on page 9".</li><li><b>Route the analytical ones out of RAG.</b> "Which region grew fastest" is not a retrieval question. Load the extracted table into a real table and answer with SQL or a calculator tool.</li></ul>',
  dgm: { nodes: ['PDF', { t: 'layout parse', s: 'detect tables' }, { t: 'table → Markdown', s: 'header preserved', k: 'alt' }, { t: 'chunk whole', s: 'or split by rows + repeat header' }, { t: 'row sentences', s: 'second index', k: 'alt' }, 'retrieve + cite cell'],
    cap: 'Every fix is at index time. Nothing at query time can restore a header that was thrown away during parsing.' },
  trap: 'The answer that fails is "increase the chunk size". A bigger chunk still contains a table with no headers attached to its cells, and it now dilutes the embedding as well. Say clearly that this is a parsing defect surfacing as a retrieval symptom.',
  tags: ['tables', 'chunking', 'parsing'],
  xref: [['Run six splitters over one document', '../genai_flow/index.html']] },

{ id: 'rg58', topic: 'rag', level: 3,
  q: 'RAG worked perfectly in production. Three months later it retrieves irrelevant results and nobody changed the code. What happened?',
  lay: 'Nothing changed in the code. Everything changed around it: more documents, different questions, a re-embedded corpus, an index full of deleted rows, a provider that quietly shipped a new version of the embedding model. A retrieval system is not a function of your code alone — it is a function of your data, your users and someone else\'s model.',
  tech: 'The candidate causes, roughly in order of how often they are the culprit:<ul><li><b>Corpus dilution.</b> The corpus tripled. Top-k is still 5, so near-duplicates and newer, wordier documents crowd out the one good chunk. Recall@5 fell without a single line changing.</li><li><b>Mixed embedding spaces.</b> Someone re-embedded part of the corpus with a newer model, or the provider silently updated theirs. Two vector spaces in one index means distances that mean nothing. This one is silent and total.</li><li><b>Index rot.</b> Months of updates and deletes leave tombstones and a degraded HNSW graph; recall drifts down until the index is rebuilt or compacted.</li><li><b>Query drift.</b> Users ask about things that did not exist three months ago — new products, new error codes — and the corpus never gained documents about them. Retrieval is fine; the corpus is stale.</li><li><b>Freshness lag.</b> The ingestion job has been failing on a subset of sources since a schema change, so the newest documents are simply not in the index.</li><li><b>Stale thresholds.</b> A fixed similarity cutoff or a reranker score gate calibrated on the old score distribution now filters good results.</li><li><b>Cache staleness</b> — a semantic cache with no version in the key still serving answers written against documents that have since changed.</li></ul><b>How to tell them apart in an hour:</b> re-run the original golden question set today. If recall@k dropped on the <i>same</i> questions, the problem is the index or the embeddings — then compare index document count against the source of truth, and check whether every vector carries the same embedding model id. If recall on the old set is fine, the problem is drift in what is being asked: sample this week\'s real queries and label twenty of them.',
  trap: 'The line that lands: "the code is only one of the inputs — the corpus, the query distribution and the provider\'s model are the other three, and two of those change without asking me." Then name the preventions: embedding model id in every row\'s metadata, a nightly golden-set run with recall@k on a dashboard, index-versus-source count monitoring, and a scheduled compaction.',
  tags: ['drift', 'production', 'debugging'],
  xref: [['Where retrieval quality actually goes', '../ai_system_design_concepts/index.html']] }
