...
{ id: 'ag46', topic: 'agents', level: 3,
  q: 'You built five specialised agents. Together they score worse than one general agent on the same base model. Why?',
  lay: 'Because splitting the work also split the context. Each specialist sees a summary of what the last one did instead of the actual evidence, every handoff loses a little, small losses multiply across five hops, and a router that sends the task to the wrong specialist is a mistake nobody downstream can undo.',
  tech: 'What actually causes it, in the order worth checking:<ul><li><b>Lossy handoffs.</b> Agent B receives a paragraph summarising what A found, not what A saw. The detail that mattered was in the part that got summarised away — and no later agent can recover it.</li><li><b>Error compounding.</b> Five hops at 90% each is 59% end to end. The specialists can each be better than the generalist and the pipeline still be worse.</li><li><b>Routing is unrecoverable.</b> A generalist that starts down the wrong path can notice and turn round. A misrouted task in a fan-out never comes back.</li><li><b>No shared state.</b> Agents developing separate beliefs about the same facts produce output that contradicts itself. This is why tasks needing shared state — coding especially — degrade under decomposition, while breadth-first search tasks improve.</li><li><b>Prompt dilution.</b> Five prompts, five chances for the original intent to be reworded into something adjacent.</li><li><b>Nothing owns the answer.</b> When no single agent is accountable for the final output, no one checks it against the original request.</li></ul><b>The fixes:</b> pass artefacts, not summaries — write to a shared scratchpad or filesystem and hand over references; give one agent ownership of the final answer with the original request in view; instrument per-hop so you can see which hop loses the quality; and collapse back to one agent with tools unless you can name the specific failure that separation fixes.',
  trap: 'The sophisticated version of this answer distinguishes the two task shapes: parallel subagents win where the work is genuinely independent and read-only — searching many sources, reviewing many files — and lose where the work shares mutable state. Say which shape the task in the question has, and the answer writes itself.',
  tags: ['multi-agent', 'architecture'],
  xref: [['Multi-agent, and when not to', '../agentic_ai_flow/index.html']] }
