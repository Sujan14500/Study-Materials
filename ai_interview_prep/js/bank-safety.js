...
{ id: 'sf29', topic: 'safety', level: 3,
  q: 'Design a system so that a competitor who steals the weights of your fine-tuned model gets nothing useful.',
  lay: 'Stop making the weights the product. Keep what is actually valuable outside the file — the live index of your data, the tools it can call, the permissions, the adapters that only load inside your own serving stack — so what they took is a generic model that knows nothing specific and can do nothing.',
  tech: 'Four layers, from the architectural to the legal:<ul><li><b>Keep the knowledge out of the weights.</b> Private facts belong in a retrieval index you own, not baked in by fine-tuning. This is the same argument as freshness and deletion rights, arriving from a different direction: non-parametric knowledge can be revoked, and a stolen model cannot bring the index with it. Fine-tune for <i>behaviour</i> — format, tone, tool discipline — and retrieve for <i>facts</i>.</li><li><b>Split the artefact.</b> Serve a commodity base model plus your own LoRA adapters. The adapters are small, can be encrypted at rest, and can be decrypted only inside an attested runtime — confidential computing on the GPU, a signed model manifest, keys released by an attestation service rather than sitting on the box. Variants: keep the output head or a small proprietary slice of layers server-side so the stolen part cannot run standalone.</li><li><b>Put the value in the harness.</b> Tools, permissions, the eval harness, the feedback flywheel, the retrieval corpus and the guardrail stack are most of the quality, and none of it is in a checkpoint. A competitor with your weights and none of that ships a worse product than yours.</li><li><b>Make theft detectable and provable.</b> Fingerprint the model with rare trigger phrases that produce a known response, and seed canary documents into the training data. Neither prevents theft; both turn "that looks like our model" into evidence.</li></ul><b>And close the realistic path.</b> Weight exfiltration is the dramatic scenario; <b>model extraction through your own API</b> — distilling your model by querying it a few million times — is the one that actually happens. Rate limits per tenant, no raw logprob exposure, anomaly detection on high-volume systematic querying, and terms that forbid training on outputs.',
  dgm: { nodes: [{ t: 'base weights', s: 'commodity' }, { t: 'LoRA adapter', s: 'encrypted, attested', k: 'alt' }, { t: 'retrieval index', s: 'never leaves', k: 'alt' }, { t: 'tools + permissions', s: 'your infra', k: 'alt' }, { t: 'answer', k: 'ok' }],
    cap: 'Steal box one and you have a public model. The product is boxes two, three and four.' },
  trap: 'Say the honest caveat out loud: nothing survives a full compromise of the infrastructure that runs the model, because the weights must be decrypted somewhere to do a forward pass. The goal is raising cost, shrinking blast radius and making theft provable — not an unbreakable box.',
  tags: ['model-theft', 'architecture'] },

{ id: 'sf30', topic: 'safety', level: 2,
  q: 'Your model passed every guardrail test and was jailbroken in production within a week. What was wrong with the testing?',
  lay: 'You tested a fixed list of attacks you thought of, once. Production is thousands of people improvising, sharing what works, in languages and encodings your list never contained — and they only need one success, while your tests only proved you survived the attempts you had already imagined.',
  tech: 'The specific gaps, each with its fix:<ul><li><b>Static suite, adaptive attacker.</b> A frozen list of prompts measures yesterday. Add automated red teaming that generates new attacks against the current system — iterative attacker-model loops, not a CSV.</li><li><b>Single-turn only.</b> Most real jailbreaks are multi-turn: establish a role, escalate gradually, then ask. Test conversations, not prompts.</li><li><b>One language, one encoding.</b> Translation, base64, leetspeak, zero-width characters and homoglyphs all route around a classifier trained on plain English.</li><li><b>You tested the model, not the system.</b> In a RAG or agent product the attack arrives inside a retrieved document or a tool result, where no one is running the input classifier. That is indirect injection, and it is the channel that gets used.</li><li><b>One layer.</b> A single output classifier is a single point of failure. Layer: input screening, prompt isolation, retrieval trust rules, output classification, and tool permissions.</li><li><b>Pass/fail instead of a rate.</b> "Passed" hides the number. Report bypass rate per attack family with a target, so a regression is visible.</li></ul><b>The structural answer:</b> assume the model will be jailbroken and design so it does not matter. If a jailbroken model can only produce embarrassing text, you have an incident; if it can issue a refund, delete a table or read another tenant\'s data, you have a breach. Least privilege at the tool boundary is the only defence that holds when the text-level ones fail.',
  trap: 'The line: "guardrails are not a test you pass, they are a control you operate." Then name the loop — every real bypass found in production becomes a test case within a day, and the suite grows for the life of the product.',
  tags: ['jailbreak', 'red-team', 'eval'],
  xref: [['The seven guardrail layers, mapped', '../agentic_ai_flow/index.html']] }
