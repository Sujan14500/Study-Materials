...
{ id: 'po41', topic: 'prodops', level: 2,
  q: 'Give me eight fixes for a slow RAG pipeline that change neither the data nor the model.',
  lay: 'Same documents, same model, same answers — just engineered properly. Most slow pipelines are slow because work happens one thing after another that could happen at the same time, because a connection is set up from scratch every request, or because the same question is answered from scratch for the hundredth time.',
  tech: '<b>Measure first</b>, then in rough order of payoff:<ol><li><b>Semantic cache.</b> Embed the incoming question, serve a stored answer when a past question is close enough. Cache hits return in tens of milliseconds instead of seconds, and on a real support workload a large share of traffic is repeat questions. Threshold measured, not guessed, and the key must carry tenant, prompt version and document versions.</li><li><b>Fewer, better chunks.</b> Ten 500-token chunks is 5,000 tokens of prefill before the first word is generated. Three well-chosen ones answer as well and prefill in a fraction of the time. This is a latency lever <i>and</i> a quality lever — context precision goes up.</li><li><b>Stream.</b> Does not reduce total time by one millisecond; moves what the user experiences from four seconds of nothing to 200 ms to first token.</li><li><b>Co-locate or self-host the embedding model.</b> A round trip to an external embedding API costs 100–150 ms on every single query. A small local model does it in single-digit milliseconds — worth it for query embedding even if indexing stays on the API.</li><li><b>Filter before you search.</b> Push tenant, date and department into the vector query as a filter rather than retrieving broadly and discarding afterwards. Searching a partition is cheaper than searching everything and throwing most of it away.</li><li><b>Rerank proportionately.</b> Cross-encoders are accurate and slow. Rerank the top 20 rather than the top 100, use a small reranker, run it on a GPU, or skip it for queries the retriever already answered confidently.</li><li><b>Parallelise everything independent.</b> Dense and lexical retrieval, multiple collections, and input guardrail classifiers all run concurrently. Sequential awaits are the most common self-inflicted latency in these pipelines.</li><li><b>Connection pooling and keep-alive.</b> A fresh TCP and TLS handshake to the vector database, cache and model provider on every request is 50–100 ms of pure overhead, repeated three times. Pool once at process start.</li></ol>Typical shape: a naive pipeline at 5–8 s, an engineered one at 1–2 s, cache hits under 100 ms.',
  dgm: { nodes: [{ t: 'cache check', s: 'hit → done', k: 'ok' }, { t: 'embed locally', s: '~8 ms' }, { t: 'dense ∥ BM25', s: 'concurrent', k: 'alt' }, { t: 'rerank top 20', s: 'not top 100' }, { t: 'stream tokens', s: 'TTFT 200 ms', k: 'ok' }],
    cap: 'Seven of the eight are free. Only "fewer chunks" trades anything, and it usually trades upward.' },
  trap: 'Say explicitly which fixes change the number and which change the feeling: streaming and perceived latency are not the same lever as caching and actual latency. And insist on tracing first — teams routinely optimise retrieval for a week when a synchronous guardrail call was the five seconds.',
  tags: ['latency', 'caching', 'rag'],
  xref: [['Latency budgets, with a live waterfall', '../ai_system_design_concepts/index.html']] }
