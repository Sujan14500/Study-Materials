...
const KEY = 'mlfund.state';

/* localStorage throws outright in some contexts — opaque origins, private
   modes with site data blocked. Progress is a nicety; losing it must never
   stop the course itself from working. */
const store = {
...
const state = Object.assign({ at: 0, seen: [], xp: 0 }, saved);
state.seen = new Set(state.seen);

const save = () => store.set(KEY, JSON.stringify({ at: state.at, seen: [...state.seen], xp: state.xp }));
...
/* This file runs at parse time, which is BEFORE demos.js builds its cards on
   ...
   navigated away and back — which is exactly what landing on chapter 1 and
   its "should this even be ML?" cards looked like. This listener is
